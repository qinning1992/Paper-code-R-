...
.p3_output_dir <- file.path(.p3_base_output_dir, "P3_only_outputs")

if(dir.exists(.p3_output_dir)){
  unlink(list.files(.p3_output_dir, full.names=TRUE, all.files=TRUE, no..=TRUE),
         recursive=TRUE, force=TRUE)
} else {
  dir.create(.p3_output_dir, recursive=TRUE)
}
output_dir <- .p3_output_dir
cat("\n[P3-only mode] Output folder cleaned:\n", .p3_output_dir, "\n")
cat("[P3-only mode] Allowed outputs:\n - ", paste(.p3_allowed_outputs, collapse="\n - "), "\n", sep="")

write.csv <- function(x, file="", ..., row.names=TRUE, fileEncoding=""){
  fn <- basename(file)
  if(!(fn %in% .p3_allowed_outputs)){
    message("[P3-only mode] Skipped non-target CSV: ", fn)
    return(invisible(NULL))
  }
  file2 <- file.path(.p3_output_dir, fn)
  utils::write.csv(x, file=file2, ..., row.names=row.names, fileEncoding=fileEncoding)
  return(invisible(file2))
}

ggsave <- function(filename, plot=ggplot2::last_plot(), device=NULL, path=NULL, ..., dpi=300){
  fn <- basename(filename)
  if(!(fn %in% .p3_allowed_outputs)){
    message("[P3-only mode] Skipped non-target figure: ", fn)
    return(invisible(NULL))
  }
  file2 <- file.path(.p3_output_dir, fn)
  ggplot2::ggsave(filename=file2, plot=plot, device=device, path=NULL, ..., dpi=dpi)
  return(invisible(file2))
}
...
ggsave(file.path(output_dir, "Fig2_A_overall_topROIs_beta.png"),
       p2, width=10, height=max(6, topN*0.28), dpi=600)
...
df_path <- res_B %>%
  mutate(
    ...
    Contrast = case_when(
      ...
      grepl("^G2_nonHS", Contrast) ~ "Other_vs_HC",
      TRUE ~ NA_character_
    )
  ) %>%
...
res3 <- res3 %>%
  mutate(
    Family = vapply(ROI, roi_family, character(1)),
    Family = factor(Family, levels=c("SubcorticalAseg","HippocampusSubfields","ThalamusNuclei","Cortex","GlobalOther"))
  )
...
ggsave(file.path(output_dir, "FigP3_heatmap_AllPAT_HS_Other_familyFDR_HedgesG_star.png"),
       p_hm, width=7, height=max(10, topN*0.22), dpi=600)
...
# ==========================================================
# STRICT FINAL OUTPUT CHECK
# Ensures the P3-only folder contains exactly the three requested files.
# ==========================================================
.final_files <- list.files(.p3_output_dir, all.files=FALSE, no..=TRUE)
.extra_files <- setdiff(.final_files, .p3_allowed_outputs)
if(length(.extra_files) > 0){
  unlink(file.path(.p3_output_dir, .extra_files), recursive=TRUE, force=TRUE)
}
.final_files <- list.files(.p3_output_dir, all.files=FALSE, no..=TRUE)
.missing_files <- setdiff(.p3_allowed_outputs, .final_files)
if(length(.missing_files) > 0){
  stop("❌ P3-only mode finished, but these required files were not generated:\n- ",
       paste(.missing_files, collapse="\n- "))
}
cat("\n✅ STRICT CHECK PASSED. The output folder contains exactly these 3 files:\n - ",
    paste(sort(.p3_allowed_outputs), collapse="\n - "),
    "\nFolder:\n", .p3_output_dir, "\n", sep="")
