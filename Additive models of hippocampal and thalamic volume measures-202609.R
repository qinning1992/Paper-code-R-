...
OUT_DIR <- "C:/Users/86150/Documents/HIPP_Thal/additive_model/ipsi-contra"
...
############################################################
## 14. Sensitivity analysis: Anatomical Left/Right models
##     Added after all original analyses
##
## Purpose:
##   Test whether the additive hippocampal–thalamic findings
##   are robust when preserving anatomical Left/Right sides
##   rather than recoding structures as ipsi/contra.
##
## IMPORTANT:
##   In pooled Left/Right analyses, seizure-onset side (side_f)
...
## ---------------------------------------------------------
## 14.1 Output directory for ALL newly added results
## ---------------------------------------------------------
ANAT_OUT_DIR <- "C:\\Users\\86150\\Documents\\HIPP_THAL\\additive_model\\Anatomical_LeftRight"
if (!dir.exists(ANAT_OUT_DIR)) dir.create(ANAT_OUT_DIR, recursive = TRUE)

cat("\n========================================\n")
cat("Starting Anatomical Left/Right sensitivity analysis\n")
cat("Output directory: ", ANAT_OUT_DIR, "\n", sep = "")
cat("========================================\n\n")


## ---------------------------------------------------------
## 14.2 Required variables and anatomical covariates
## ---------------------------------------------------------
required_anat_vars <- c(
  "side_lr",
  "Hippo_Left_Volume",
  "Hippo_Right_Volume",
  "Thal_Left_SummedVolume",
...
missing_anat_vars <- setdiff(required_anat_vars, names(df_pat))
if (length(missing_anat_vars) > 0) {
  stop(
    "Anatomical Left/Right sensitivity analysis is missing variables:\n- ",
    paste(missing_anat_vars, collapse = "\n- ")
  )
}

## Preserve anatomical side and adjust for seizure-onset side
## in the pooled Left/Right sensitivity models.
df_pat$side_f <- factor(df_pat$side_lr, levels = c("L", "R"))

cat("Seizure-onset side counts used in sensitivity analysis:\n")
print(table(df_pat$side_f, useNA = "ifany"))
...
cat(
  "Anatomical Left/Right covariates: ",
  paste(ANATOMICAL_COVARS, collapse = " + "),
  "\n\n",
  sep = ""
)
...
## ---------------------------------------------------------
## 14.3 Four Anatomical Left/Right sensitivity model sets
##
## M0 = covariates
## M1 = covariates + hippocampus
## M2 = covariates + thalamus
## M3 = covariates + hippocampus + thalamus
## ---------------------------------------------------------
anatomical_model_sets <- list(
  list(
    set_name = "Sensitivity_Anatomical_Left_Global",
    hippo_var = "Hippo_Left_Volume",
    thal_var = "Thal_Global_SummedVolume"
  ),
  list(
    set_name = "Sensitivity_Anatomical_Right_Global",
    hippo_var = "Hippo_Right_Volume",
    thal_var = "Thal_Global_SummedVolume"
  ),
  list(
    set_name = "Sensitivity_Anatomical_Left_Left",
    hippo_var = "Hippo_Left_Volume",
    thal_var = "Thal_Left_SummedVolume"
  ),
  list(
    set_name = "Sensitivity_Anatomical_Right_Right",
    hippo_var = "Hippo_Right_Volume",
    thal_var = "Thal_Right_SummedVolume"
  )
)


## ---------------------------------------------------------
## 14.4 Run the four sensitivity model sets
## ---------------------------------------------------------
anatomical_results_list <- list()

for (ii in seq_along(anatomical_model_sets)) {
  ms <- anatomical_model_sets[[ii]]
  
  cat(
    "Running Anatomical Left/Right model: ",
    ms$set_name,
    "\n",
    sep = ""
  )
  
  anatomical_results_list[[ms$set_name]] <- run_step3_one_set(
    df_pat = df_pat,
    outcomes = OUTCOMES_PRIMARY,
    main_covars = ANATOMICAL_COVARS,
    hippo_var = ms$hippo_var,
    thal_var = ms$thal_var,
    set_name = ms$set_name,
    n_perm = N_PERM,
...
anatomical_left_right_results <- do.call(
  rbind,
  anatomical_results_list
)

write_csv_cn(
  anatomical_left_right_results,
  "volume_sensitivity_anatomical_LeftRight_all_results.csv",
  out_dir = ANAT_OUT_DIR
)


############################################################
## 15. N diagnostics and collinearity diagnostics
############################################################

anat_n_diag <- list()
anat_col_diag <- list()

for (ms in anatomical_model_sets) {
  ...
  anat_n_diag[[ms$set_name]] <- tmp_n
  
  
  tmp_col <- t(sapply(OUTCOMES_PRIMARY, function(y) {
    check_collinearity_fun(
      df_pat,
      y,
      ANATOMICAL_COVARS,
      ms$hippo_var,
  ...
  anat_col_diag[[ms$set_name]] <- tmp_col
}

anatomical_n_diagnostics <- do.call(rbind, anat_n_diag)
anatomical_collinearity_diagnostics <- do.call(rbind, anat_col_diag)

write_csv_cn(
  anatomical_n_diagnostics,
  "volume_sensitivity_anatomical_LeftRight_n_diagnostics.csv",
  out_dir = ANAT_OUT_DIR
)

write_csv_cn(
  anatomical_collinearity_diagnostics,
  "volume_sensitivity_anatomical_LeftRight_collinearity.csv",
  out_dir = ANAT_OUT_DIR
)


############################################################
## 16. Wide summary tables
############################################################

get_anatomical_set_table <- function(df, set_name, suffix) {
...
anat_summary_wide <- get_anatomical_set_table(
  anatomical_left_right_results,
  "Sensitivity_Anatomical_Left_Global",
  "LeftGlobal"
)

anat_summary_wide <- merge(
  anat_summary_wide,
  get_anatomical_set_table(
    anatomical_left_right_results,
...
anat_summary_wide <- merge(
  ...
  get_anatomical_set_table(
    anatomical_left_right_results,
    "Sensitivity_Anatomical_Left_Left",
    "LeftLeft"
  ),
  by = "Outcome",
  all = TRUE
...
anat_summary_wide <- merge(
  ...
  get_anatomical_set_table(
    anatomical_left_right_results,
    "Sensitivity_Anatomical_Right_Right",
    "RightRight"
  ),
  by = "Outcome",
  all = TRUE
...
write_csv_cn(
  anat_summary_wide,
  "volume_sensitivity_anatomical_LeftRight_summary_wide.csv",
  out_dir = ANAT_OUT_DIR
)

KEY_OUTCOMES_ANAT <- intersect(
  c(
    "工作记忆指数WMI",
    "总记忆商",
    "听觉记忆指数",
...
anat_summary_for_manuscript <- anatomical_left_right_results %>%
  filter(Outcome %in% KEY_OUTCOMES_ANAT) %>%
  select(
    Set,
    Outcome,
    HippoVar,
...
write_csv_cn(
  anat_summary_for_manuscript,
  "volume_sensitivity_anatomical_LeftRight_summary_for_manuscript.csv",
  out_dir = ANAT_OUT_DIR
)


############################################################
## 17. Direct comparison table:
##     original lesion-aligned primary models vs anatomical
##     Left/Right sensitivity models
############################################################

if (exists("step3_all_sets_results")) {
  
  original_primary_for_compare <- step3_all_sets_results %>%
    filter(Set %in% c(
      "Primary_Ipsi_Global",
      "Primary_Contra_Global"
    ))
  
  alignment_sensitivity_comparison <- bind_rows(
    original_primary_for_compare,
    anatomical_left_right_results
  ) %>%
    arrange(Outcome, Set)
  
  write_csv_cn(
    alignment_sensitivity_comparison,
    "volume_sensitivity_alignment_vs_anatomical_comparison.csv",
    out_dir = ANAT_OUT_DIR
  )
}
...
############################################################
## 18. Export anatomical structural variables used
############################################################
...
anat_derived_vars <- intersect(anat_derived_vars, names(df_pat))

write_csv_cn(
  df_pat[, anat_derived_vars, drop = FALSE],
  "volume_sensitivity_anatomical_LeftRight_derived_indices.csv",
  out_dir = ANAT_OUT_DIR
)


############################################################
## 19. Anatomical Left/Right sensitivity figure
##     Same Delta-R2 logic as the original additive-model plot
##     Arial + bold for reliable Windows rendering
############################################################
...
set_map_anat_fig <- c(
  "Sensitivity_Anatomical_Left_Global" =
    "Anatomical Left\nHippo: left volume; Thal: global summed volume",
  
  "Sensitivity_Anatomical_Right_Global" =
    "Anatomical Right\nHippo: right volume; Thal: global summed volume",
  
  "Sensitivity_Anatomical_Left_Left" =
    "Anatomical Left\nHippo: left volume; Thal: left summed volume",
  
  "Sensitivity_Anatomical_Right_Right" =
    "Anatomical Right\nHippo: right volume; Thal: right summed volume"
)

anat_plot_data <- anatomical_left_right_results %>%
  mutate(
    Outcome = recode(Outcome, !!!outcome_map_anat),
    Set = factor(
      recode(Set, !!!set_map_anat_fig),
      levels = unname(set_map_anat_fig)
    )
  ) %>%
  select(
    Set,
  ...
  mutate(
    ...
    Label_text = case_when(
      is.na(P_value) | is.na(Q_value) ~ "",
      TRUE ~ sprintf("P=%.3f, Q=%.3f%s", P_value, Q_value, Significance)
    )
  ) %>%
  mutate(
    Label_text = gsub("P=0.000", "P<0.001", Label_text),
...
anat_max_delta <- max(anat_plot_data$DeltaR2, na.rm = TRUE)
if (!is.finite(anat_max_delta) || anat_max_delta <= 0) anat_max_delta <- 0.10

p_anatomical_sensitivity <- ggplot(
  anat_plot_data,
  aes(
    x = reorder(Outcome, DeltaR2),
    y = DeltaR2,
    fill = Comparison
  )
) +
  ...
  geom_text(
    aes(
      label = Label_text,
      y = DeltaR2 + anat_max_delta * 0.018
    ),
    position = position_dodge(width = 0.82),
    size = 3.5,
    color = "black",
  ...
  coord_flip() +
  scale_y_continuous(
    expand = expansion(mult = c(0, 0.50))
  ) +
  scale_fill_manual(values = anat_comparison_colors) +
  facet_wrap(~ Set, ncol = 2, scales = "free_x") +
  labs(
    title = "Anatomical Left/Right Sensitivity Analysis",
    subtitle = paste0(
      "Additive hippocampal-thalamic models; seizure-onset side adjusted. ",
      "Permutation tests = ", N_PERM, "."
    ),
    x = "Cognitive and Memory Domains",
    y = expression("Incremental Explained Variance (" * Delta * R^2 * ")")
  ) +
  ...
  theme(
    ...
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
    plot.subtitle = element_text(size = 10.5, face = "bold", hjust = 0.5),
...
ggsave(
  file.path(
    ANAT_OUT_DIR,
    "VolumeSensitivity_Anatomical_LeftRight_Incremental_R2_Panel.png"
  ),
  plot = p_anatomical_sensitivity,
  width = 14,
  height = 14,
  dpi = 600,
  bg = "white"
...
## Cairo PDF is more stable for Arial on Windows.
ggsave(
  file.path(
    ANAT_OUT_DIR,
    "VolumeSensitivity_Anatomical_LeftRight_Incremental_R2_Panel.pdf"
  ),
  plot = p_anatomical_sensitivity,
  width = 14,
  height = 14,
...
cat("\n========================================\n")
cat("Anatomical Left/Right sensitivity analysis completed.\n")
cat("Output directory: ", ANAT_OUT_DIR, "\n", sep = "")
cat("Generated files:\n")
cat("1. volume_sensitivity_anatomical_LeftRight_all_results.csv\n")
cat("2. volume_sensitivity_anatomical_LeftRight_n_diagnostics.csv\n")
cat("3. volume_sensitivity_anatomical_LeftRight_collinearity.csv\n")
cat("4. volume_sensitivity_anatomical_LeftRight_summary_wide.csv\n")
cat("5. volume_sensitivity_anatomical_LeftRight_summary_for_manuscript.csv\n")
cat("6. volume_sensitivity_alignment_vs_anatomical_comparison.csv (if original results exist)\n")
cat("7. volume_sensitivity_anatomical_LeftRight_derived_indices.csv\n")
cat("8. VolumeSensitivity_Anatomical_LeftRight_Incremental_R2_Panel.png\n")
cat("9. VolumeSensitivity_Anatomical_LeftRight_Incremental_R2_Panel.pdf\n")
cat("========================================\n\n")
